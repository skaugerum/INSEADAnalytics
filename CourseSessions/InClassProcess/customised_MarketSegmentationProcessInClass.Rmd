...
thecor[abs(thecor) < MIN_VALUE]<-NA
# which(abs(thecor) > MIN_VALUE & thecor < 1, arr.ind=TRUE)
# thecor[which(abs(thecor) > MIN_VALUE & thecor < 1, arr.ind=TRUE)]
...
1. Setting the "high-water mark" for correlations at |0.5| yields 11 pairs, all fall between 0.50 and 0.64. By looking at the pairs, we can see that all these correlations make sense.
    - Q1.3 & Q1.9     status
    - Q1.6 & Q1.7     reward
    - Q1.9 & Q1.26    status
    - Q1.13 & Q1.14   latest and greatest
    - Q1.14 & Q1.15   latest and greatest
    - Q1.16 & Q1.17   boat freak
    - Q1.16 & Q1.20   boat freak
    - Q1.16 & Q1.28   boat freak
    - Q1.20 & Q1.27   boat freak
    - Q1.20 & Q1.28   boat freak
    - Q1.27 & Q1.28   boat freak

2. When reducing the threshold to |0.45|, the number of pairs increases to 25. Many of the same patterns emerge, centered around status, reward, fashion and boat freak. This indicates that the 29 questions probably can be reduced to a smaller number of factors to classify and describe a few segments.
...
1. The table shows the **Principal Component Analysis** of all 29 questions with the respective *eigenvalues*, *% of explained variance*, and *Cumulative % of explained variance*, sorted descending by the component (question) with the highest eigenvalue, where
$$\displaystyle \frac{\sum_{i=1}^{n} eigenvalue_i}{\ n} = {\%\;of\;explained\;variance}$$
Hence, the eigenvalue corresponds to how much variance is caputred in the component. 
The graph plots the eigenvalues in the table together with an abline. The abline shows a threshold (in this case = 1) that can be used to choose the number of components to include when segmenting. 
2. As the eigenvalues are sorted from highest to lowest with a minimum value of 0, the graph will always be converging to zero with an exponential decay shape. As long as the table is sorted descending, the line can not be increasing. (Note: the appearing increase from component 2 to 3 is because of the smoothed line).
3. We want to see an "elbow" shaped plot (as we do) for two reasons:
    - It shows that some components have high eigenvalues and some have low eigenvalues, which indicates that we can reduce the number of components
    - The elbow point can be used to determine the number of components to include in the segmentation
...
1. Analyzing the first 5 factors, we find that
    - Component 1 indicates status and knowledge -- people who are boat freaks, who want the latest and greatest, see the boat as a status symbol, and spend a lot of time on boating activities.
    - Component 2 indicates leisure -- people who use their boat to socialize, seek adventure and reward themselves.
    - Component 3 indicates social status -- people who care about the brand of the boat and what that brand signals.
    - Component 4 indicates geeks -- people who know more than most other about boats and make DYI adjustments to their boats.
    - Component 5 indicates low budget -- people who want a functional boat at a low price

All factors make business sense, however there is some overlap between component 1 and 2-4, indicating that component 1 is a little too large. It also contains more questions than all the other.
***NEED TO PLAY AROUND WITH NUMBER OF COMPONENTS! Suggest 5 (eigenvalue > 1), 6 (elbow point) and 9 (cum. % <= 65%)***

2. We suggest using **X** factors for the following reasons:
    a) 
    b)
    c)
